  ...
  module Api
    module Oauth
      ...
      def qrcode_authorize_url(redirect_uri, scope="snsapi_login", state="web_wx_login")
        ...
        WeixinmpApi.open_endpoint("/connect/qrconnect?appid=#{app_id}&redirect_uri=#{uri}&response_type=code&scope=#{scope}&state=#{state}#wechat_redirect")
      end
      ...
      def authorize_url(redirect_uri, scope="snsapi_base", state="weixin")
        uri = encode_url(redirect_uri)
        WeixinmpApi.open_endpoint("/connect/oauth2/authorize?appid=#{app_id}&redirect_uri=#{uri}&response_type=code&scope=#{scope}&state=#{state}#wechat_redirect")
      end
      ...
      def get_oauth_access_token(code)
        WeixinmpApi.http_get_without_token("/sns/oauth2/access_token?appid=#{app_id}&secret=#{app_secret}&code=#{code}&grant_type=authorization_code", {}, "api")
      end

      # refresh_token: 填写通过access_token获取到的refresh_token参数
      def refresh_oauth2_token(refresh_token)
        WeixinmpApi.http_get_without_token("/sns/oauth2/refresh_token?appid=#{app_id}&grant_type=refresh_token&refresh_token=#{refresh_token}", {}, "api")
      end

      # 如果网页授权作用域为snsapi_userinfo，则此时开发者可以通过access_token和openid拉取用户信息了。
      def get_oauth_userinfo(openid, oauth_token, lang="zh_CN")
        WeixinmpApi.http_get_without_token("/sns/userinfo?access_token=#{oauth_token}&openid=#{openid}&lang=#{lang}", {}, "api")
      end
